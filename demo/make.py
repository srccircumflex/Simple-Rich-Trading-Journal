...
def make():
    ...
    def deposit():
        ...
        _money.cash += amount
        _deposits.append(Deposit({"n": 0, "InvestAmount": amount, "InvestTime": time.strftime(time_formats.transaction_time_format)}, time, amount, now))

    def payout():
        if (x := _money.cash // 100) > 10:
            amount = (randint(1, int(x) - 9) / 5) * 100
            _money.cash -= amount
            _payouts.append(Payout({"n": 0, "TakeAmount": amount, "TakeTime": time.strftime(time_formats.transaction_time_format)}, time, amount, now))
    ...
    def trading(lo=True):
        if _open_trades and randint(0, (x := len(_open_trades)) + 12) > 11:
            ...
            _money.cash += amount
            _finalized_trades.append(TradeFinalized(trade.data | {"TakeAmount": amount, "TakeTime": time.strftime(time_formats.transaction_time_format)}, time, amount, now))
            ...
            for i in range(randint(3, 8)):
                ...
                asset[-1] *= randrate()
            _money.cash -= amount
            _open_trades.append(TradeOpen({"n": n, "Name": asset[0], "Symbol": asset[2], "Type": asset[1], "InvestAmount": amount, "InvestTime": time.strftime(time_formats.transaction_time_format)}, time, amount, now, False))
        else:
            deposit()
    ...
    def dividend():
        if _open_trades:
            ...
            _money.cash += amount
            _dividends.append(Dividend({"n": 0, "Name": trade["Name"], "TakeAmount": amount, "TakeTime": time.strftime(time_formats.transaction_time_format)}, time, amount, now, False))
        else:
            trading()
    ...
    def itc():
        if (x := _money.cash // 100) > 1:
            ...
            _money.cash += amount
            if randint(0, 10) > 3:
                _itc.append(Itc({"n": 0, "ITC": amount, "InvestTime": time.strftime(time_formats.transaction_time_format)}, time, amount, now, True))
            else:
                _itc.append(Itc({"n": 0, "ITC": amount, "TakeTime": time.strftime(time_formats.transaction_time_format)}, time, amount, now, False))
        else:
            deposit()
