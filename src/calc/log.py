...
import plugin
from src.calc.utils import datetime_from_tradetimeformat, tradetimeparser, preventZeroDiv
...
class Deposit(_LogRecord):
    ...
    @cached_property
    def data(self) -> dict:
        keys = ("id", "cat", "n", "InvestTime", "InvestAmount", "ITC", "Note")
        self.row_dat = {k: self.row_dat.get(k) for k in keys}
...
class Payout(_LogRecord):
    ...
    @cached_property
    def data(self) -> dict:
        keys = ("id", "cat", "n", "TakeTime", "TakeAmount", "ITC", "Note")
        self.row_dat = {k: self.row_dat.get(k) for k in keys}
        self["Name"] = record_tags.tag_PAYOUT
        self["Performance"] = -preventZeroDiv(self.real_amount, self.base_value, None)
        return self.row_dat
...
class _Trade(_LogRecord):
    ...
    @cached_property
    def data(self) -> dict:
        keys = ("id", "cat", "Name", "Symbol", "Type", "n", "InvestTime", "InvestAmount", "InvestCourse", "TakeTime", "TakeAmount", "TakeCourse", "ITC", "Note")
        self.row_dat = {k: self.row_dat.get(k) for k in keys}
...
class Dividend(_LogRecord):
    ...
    @cached_property
    def data(self) -> dict:
        keys = ("id", "cat", "Name", "n", "TakeTime", "Note")
        self.row_dat = {k: self.row_dat.get(k) for k in keys}
        if self.itc:
            self["ITC"] = self.itc
...
class Itc(_LogRecord):
    ...
    @cached_property
    def data(self) -> dict:
        keys = ("id", "cat", "n", "ITC", "Note") + (("InvestTime",) if self.at_invest else ("TakeTime",))
        self.row_dat = {k: self.row_dat.get(k) for k in keys}
...
class LogCalc:
    ...
    def getTradeFrame(
        ...
        return TradeFrameCalc(self, first, last, by_attr, ui)
...
class TradeFrameCalc(LogCalc):
    ...
    def getTradeFrame(self, first: datetime, last: datetime, by_attr: Literal["idx", "min", "max", "or"] | str = "idx", ui: bool = False) -> LogCalc:
        return LogCalc.getTradeFrame(self.__mainFrame__, first, last, by_attr, ui)
